...
def test_kwd_three_decimals_in_totals():
    ...
    # Create KWD account with 3-decimal precision
    ...
    # +1.234 and +0.001 income, -0.002 expense => 1.233
    assert client.post("/fin/transactions", json={"account_id": acc, "category_id": inc_cat, "amount": "1.234", "occurred_at": now}).status_code == 201
    assert client.post("/fin/transactions", json={"account_id": acc, "category_id": inc_cat, "amount": "0.001", "occurred_at": now}).status_code == 201
    assert client.post("/fin/transactions", json={"account_id": acc, "category_id": exp_cat, "amount": "0.002", "occurred_at": now}).status_code == 201
